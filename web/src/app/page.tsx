...
//   const highlight = post.isFeatured && post.featuredUntil && new Date(post.featuredUntil) > new Date();
//   return (
//     <article className="group">
//       <div className="bg-gray-200 h-40 w-full mb-3 rounded-xl overflow-hidden">
//         {post.coverImageUrl ? (
...
//         {highlight ? 'Urgente' : post.tags?.[0] ?? 'Geral'}
//       </span>
//       <h3 className="text-base font-bold leading-snug mt-1 line-clamp-2">
//         {post.title}
//       </h3>
...
//         <FaRegClock /> <span>{timeAgo(published, now)}</span>
//       </div>
//     </article>
//   );
// }
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const data = await fetchNews({ page, limit: LIMIT });
        if (cancel) return;
  ...
  // const gridMain = useMemo(() => posts.slice(0, 3), [posts]);
          ...
          <ul className="space-y-4">
            ...
            {!loading && latestList.length === 0 && !headline ? (
              <li className="text-sm text-gray-500">
                {error ? "Erro ao carregar notícias." : "Nenhuma notícia publicada ainda."}
              </li>
            ) : null}
          </ul>
        </aside>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          {others.length === 0 && !loading ? (
            <div className="text-sm text-gray-500">
              {error ? "Erro ao carregar notícias." : posts.length === 0 ? "Nenhuma notícia publicada ainda." : "Sem mais notícias."}
            </div>
          ) : null}
        </div>
      </section>
